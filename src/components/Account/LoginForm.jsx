  ...
  const handleResetPasswordSubmit = (e) => {
    ...
    if (newPassword !== newPasswordConfirm) {
      ...
      fetch('http://localhost:3308/api/reset-password', {
        ...
        body: JSON.stringify({ email: emailPswReset, newPassword: newPassword }),
      })
        .then((response) => {
          if (response.ok) {
  ...
  return (
    ...
    {showOtpForm === false && isActiveResetForm === false ? (
      <form onSubmit={handleSubmit}>
          ...
          <h4 className='pswForget' onClick={()=>setIsActiveReset(!isActiveReset)}>Did you forget your password?</h4>
          </div>
      ...
      <form onSubmit={handleResetPasswordSubmit}>
        ...
        <div className={`resetPswContainer ${isActiveReset?"active":""}`}>
          ...
          <input
            ...
            required
            />
          <button onClick={handlePasswordReset} className='loginBtn' disabled={disableBtn}>Send</button>
          {disableBtn && (
